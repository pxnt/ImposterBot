// ==UserScript==
// @name         Dark ImposterBot
// @namespace    jrwr.io
// @version      1.2
// @description  A bot that randomly chooses a entry and reports back to a central database at spacescience.tech
// @author       dimden (https://dimden.dev/), jrwr (http://jrwr.io/), px, qqii, cg || modified by px to only select humans.
...
async function play() {
    ...
    for (let i = 0; i < room.options.length; i++) {
        ...
        if (abra[i] === "known human" || space[i] === "known human") {
            answer = i;
            break;
        } else if (abra[i] === "known fake" || space[i] === "known fake") {
            continue;
...
Losses: ${loses.length} (${((loses.length/(wins.length+loses.length))*100).toFixed(1)}%),
Wins: ${wins.length} (${((wins.length/(wins.length+loses.length))*100).toFixed(1)}%), Time (ms): ${avg}
`;
}
