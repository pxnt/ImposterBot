// ==UserScript==
// @name         Reddit April Fools Imposter Bot Edited to Choose Humans
// @namespace    jrwr.io
// @version      1.0.10
...
async function play() {
    ...
    let result = await submitAnswer(room.token, room.options[q][0]);

    return [room.options[q][1], result.result, room];
};
...
Wins: ${0} (${(1.1)}%)
Loses: ${wins.length+loses.length} (${(98.9)}%)
`;
}
...
setInterval(() => {
    ...
    if(gameCount === 0){
        document.getElementsByTagName("gremlin-meta")[0].innerHTML = "";
        document.getElementsByTagName("gremlin-result")[0].innerHTML = "Please leave on and do not refresh. Auto-detecting and clicking humans.";
        document.getElementsByTagName("gremlin-stats")[0].innerHTML = "";
        document.getElementsByTagName("gremlin-set_flair")[0].innerHTML = "";
        gameCount++;
    }
}, 1000);
